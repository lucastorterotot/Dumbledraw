...
class Plot(object):
    ...
    def DrawCMS(self):
        styles.DrawCMSLogo(self._subplots[0]._pad, 'CMS', 'Preliminary', 11, 0.045, 0.05, 1.0, '', 1.0)
...
class Subplot(object):
    def __init__(self, name, lower_bound = 0.0, upper_bound = 1.0):
        logger.debug("Booking subplot with lower boundary at %f and upper boundary at %f"%(lower_bound, upper_bound))
        self._pad = R.TPad("pad_"+str(name), "pad_"+str(name), 0., 0., 1., 1.)
        '''
        if lower_bound==0.0:
    ...
    def add_hist(self, hist, name, group_name="invisible"): 
        ...
        if not isinstance(hist, R.TH1F):
            ...
            raise Exception
        self._hists[name]=[copy.deepcopy(hist), group_name, ""] # last entry is used to save the markerstyle and set in a different function
    ...
    def DrawSingle(self, hist, isFirst):
        ...
        if isFirst:
            ...
            if self._ylims!=None and isinstance(hist[0], R.THStack): # otherwise lims are not set without a unintended margin
    ...
    def setAxisStyles(self, hist):
        ...
        if self._xlabel==None:
            ...
            hist.GetXaxis().SetTitleOffset(self._xtitleoffsetscale*hist.GetXaxis().GetTitleOffset())
            hist.GetXaxis().SetLabelOffset(self._xlabeloffsetscale*hist.GetXaxis().GetLabelOffset())
        if self._ylabel==None:
            ...
            hist.GetYaxis().SetTitleOffset(self._ytitleoffsetscale*hist.GetYaxis().GetTitleOffset())
            hist.GetYaxis().SetLabelOffset(self._ylabeloffsetscale*hist.GetYaxis().GetLabelOffset())
            
        # set label sizes
        if self._xlabelsize!=None:
            hist.GetXaxis().SetLabelSize(self._xlabelsize*hist.GetXaxis().GetLabelSize())
        if self._ylabelsize!=None:
            hist.GetYaxis().SetLabelSize(self._ylabelsize*hist.GetYaxis().GetLabelSize())
        if self._xtitlesize!=None:
            hist.GetXaxis().SetTitleSize(self._xtitlesize*hist.GetXaxis().GetTitleSize())
        if self._ytitlesize!=None:
            hist.GetYaxis().SetTitleSize(self._ytitlesize*hist.GetYaxis().GetTitleSize())
        ...
        for hist_name in hist_names: 
            if hist_name in self._hists.keys():
                ...
                for key, hist in self._hists.iteritems():
                    if hist_name==hist[1]:
                        if isinstance(hist[0], R.THStack):
                            logger.fatal("Tried to import a stack into a stack, which is impossible!")
                            raise Exception
                        stack.Add(hist[0])
