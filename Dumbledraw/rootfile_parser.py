...
class Rootfile_parser(object):
    def __init__(self, inputrootfilename):
        ...
        for entry in content:
            if entry.endswith("postfit"):
                self._type = "postfit"
        logger.debug("Identified rootfile %s as %s shapes"%(inputrootfilename, self._type))
    ...
    def get(self, channel, category, process, syst=None):
        if syst != None and self._type != "control":
            ...
            raise Exception
        hist_hash = "{channel}_{category}{plottype}/{process}{unc}".format(channel=channel, category=category, process=process, plottype="{plottype}", unc="{unc}")
